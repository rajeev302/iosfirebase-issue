...
class CommentListViewmodel(application: Application): AndroidViewModel(application), CoroutineScope {
    ...
    fun getCommentList(context: Context, commentNumber: String, issueNodeId: String, success: () -> Unit, failure: (message: String) -> Unit) = launch {
        repository.getCommentList(context, commentNumber, issueNodeId)?.let {
            commentList.clear()
